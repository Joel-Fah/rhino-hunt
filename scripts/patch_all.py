...
#define TAIL3 3000
"""
if globals_old in text:
    text = text.replace(globals_old, globals_new)
else:
    print("Cannot find globals_old")

# 3. Modify get_word to support tail_on
get_word_old = r"""                        if (mode > 1 && !get_code_bit(0))
                                continue;
                }
...
extract_old = r"""        if (memcmp(iv2 + 4, p + 12, 4))
                return (0);
